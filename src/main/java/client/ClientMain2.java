...
public class ClientMain2 {
    public static void main(String[] args) {
        ...
        for (int i = 1; i <= maxRetries; i++) {
            ...
            // Init output first to avoid Deadlock (main.java.server and main.java.client will be waiting header for each other => all hang)
            try (Socket socket = new Socket(Const.host, Const.port);
                ...
                if (firstRes.getStatusCode() == StatusCode.SERVICE_UNAVAILABLE) {
                    ...
                    System.out.println("✅ Connected to main.java.server successfully!");
                    System.out.println(firstRes.getMessage());
                    System.out.printf(Const.GREEN + Const.cat + Const.RESET);

                    // 2. Send Header instantly to main.java.server to get handshake response
