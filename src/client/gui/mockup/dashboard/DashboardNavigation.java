...
import client.gui.pages.dashboard.prototype.DashboardSection;
...
public class DashboardNavigation extends VBox {
    private final Consumer<client.gui.pages.dashboard.prototype.DashboardSection> onSectionChanged;
    ...
    public DashboardNavigation(String currentUser,
                               Consumer<client.gui.pages.dashboard.prototype.DashboardSection> onSectionChanged,
        ...
        treeView.getSelectionModel().selectedItemProperty().addListener((obs, old, selected) -> {
            ...
            client.gui.pages.dashboard.prototype.DashboardSection section = selected.getValue().section();
    ...
    public void refreshLanguage() {
        client.gui.pages.dashboard.prototype.DashboardSection selectedSection = selectedSection();
    ...
    private void buildTree() {
        collectionItem = leaf(Messages.get(Messages.Key.COLLECTION), client.gui.pages.dashboard.prototype.DashboardSection.COLLECTION,
                "Object table is shown in the center area");
        commandsItem = leaf(Messages.get(Messages.Key.COMMANDS), client.gui.pages.dashboard.prototype.DashboardSection.COMMANDS,
                "Commands are available through the bottom action bar");
        visualizationItem = leaf(Messages.get(Messages.Key.VISUALIZATION), client.gui.pages.dashboard.prototype.DashboardSection.VISUALIZATION,
                "Visualization area is shown in the center area");
        settingsItem = leaf(Messages.get(Messages.Key.SETTINGS), client.gui.pages.dashboard.prototype.DashboardSection.SETTINGS,
    ...
    private TreeItem<NavNode> leaf(String title, client.gui.pages.dashboard.prototype.DashboardSection section, String statusText) {
        return new TreeItem<>(new NavNode(title, section, statusText));
    }

    private client.gui.pages.dashboard.prototype.DashboardSection selectedSection() {
        TreeItem<NavNode> selected = treeView.getSelectionModel().getSelectedItem();
        if (selected == null || selected.getValue() == null || selected.getValue().section() == null) {
            return client.gui.pages.dashboard.prototype.DashboardSection.COLLECTION;
        }
