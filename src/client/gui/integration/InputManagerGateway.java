...
import client.auth.Account;
import client.gui.model.CarUiModel;
import client.gui.model.CoordinatesUiModel;
import client.gui.model.HumanBeingUiModel;
import client.gui.model.WeaponTypeUi;
...
public class InputManagerGateway implements Lab7CommandGateway {
     ...
     * Throws IOException if the server does not respond — App decides the fallback behavior.
     */
    public static InputManagerGateway connect(String host, int port) throws IOException {
        ...
        // Drain handshake sent by the server immediately upon client connection
    ...
    public List<HumanBeingUiModel> show() {
        try {
            ...
            return List.of(); // old server returns text, not a List — UI displays empty
        } catch (IOException e) {
            return List.of();
        }
    }
    ...
    public HumanBeingUiModel update(long id, HumanBeingUiModel model) {
        if (model == null) return null;
        try {
            // Step 1 — check ID (InputManager sends a null object, server returns CONTINUE)
            Response check = sender.sendRequest(
                new Request("update", String.valueOf(id), null)
            );
            if (check.getStatusCode() != common.StatusCode.CONTINUE) return null;
    ...
    public int clearMine() {
        try {
            // Step 1 — InputManager sends "clear", server returns CONTINUE + asks for confirmation
            Response check = sender.sendRequest(new Request("clear", null, null));
            if (check.getStatusCode() != common.StatusCode.CONTINUE) return 0;
